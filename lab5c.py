...
Notes:
    I have mainly focus to solve all problems using regex. 
    As we discussed during mentoring, I have used both re.finditer() and re.findall()
    in some cases findall has limitations. specially for question 8 and 7. 
    reding the txt line by line and stor it in some data structure could be an option.
    but, it wouldnot be much fun as i do to plat with regex. 
    I case if i supposed to do it only using find all, please consider, the file named Lab5a.
    Since it is diffcult to compare long lists,  
    I have coded the given listes on the assignment txt to validate my results,  
...
regexNameEmailId = r"\b(.+)\s([-+.0-9A-Z_a-z]+@[-+.0-9_a-z]+\.[A-Za-z]{2,4})\s(fs:[0-9]{3}:[0-9]{5,7})\t"
regexlastactivity = r"\b(.+)\s([-+.0-9A-Z_a-z]+@[-+.0-9_a-z]+\.[A-Za-z]{2,4})\s(fs:[0-9]{3}:[0-9]{5,7})\t\n(.+)\n(.+)\n([-+0-9]{1,2}\s[-+A-Za-z]{3}\sat\s[-+0-9]{1,2}:[-+0-9]{1,2})\n"
regexTotallActivity=r"\b(.+)\s([-+.0-9A-Z_a-z]+@[-+.0-9_a-z]+\.[A-Za-z]{2,4})\s(fs:[0-9]{3}:[0-9]{5,7})\t\n(.+)\n(.+)\n([-+0-9]{1,2}\s[-+A-Za-z]{3}\sat\s[-+0-9]{1,2}:[-+0-9]{1,2})\n([-+0-9]{1,2}:[-+0-9]{1,2}:[-+0-9]{1,2}|[-+0-9]{1,2}:[-+0-9]{1,2})\b"

matches = re.finditer(regexNameEmailId, test_str, re.MULTILINE)
question1=[] # oslomet emails Done
question2=[] #any email , Done
question3=[] #last activity eg 13 Sep at 16:57
question4=[]    #sys ID fs:215:1032851 Done
question5=[]    #Tottal activity times 07:59:54
question6=[]    #name of student ... Done
...
for matchNum, match in enumerate(matches, start=1):
    ...
    if (re.search('[-+.0-9A-Z_a-z]+@oslomet.no', match.group(2))):
        question1.append(match.group(2))
    
    question2.append(match.group(2))
    
    question4.append(match.group(3))
    
    question6.append(match.group(1))
    

matches = re.finditer(regexlastactivity, test_str, re.MULTILINE)

for matchNum, match in enumerate(matches, start=1):     
    if(re.search('Aug at', match.group(6))):
       question8.append(match.group(1))
       
matches = re.finditer(regexTotallActivity, test_str, re.MULTILINE)

for matchNum, match in enumerate(matches, start=1):     
    if(re.search('^[1-9]', match.group(7))):
       question7.append(match.group(1))

question5=re.findall('[-+0-9]{1,2}:[-+0-9]{1,2}:[-+0-9]{1,2}|[-+0-9]{1,2}:[-+0-9]{1,2}', test_str)
question3 = re.findall("[-+0-9]{1,2}\s[-+A-Za-z]{3}\sat\s[-+0-9]{1,2}:[-+0-9]{1,2}", test_str)
...
#Since it is diffcult to compare long lists,  I have coded the given listes on the assignment txt to validate my results,  
print('Test with Given examples in the assignment text')
...
if len(listdiff)==0: 
    ...
    print('The difference question 1=', listdiff)
    

listdiff = [item for item in allemails if item not in question2]
if len(listdiff)==0: 
    print('question 2 done')
else:
    print('The difference question 2=', listdiff)
    

listdiff = [item for item in lastActivity if item not in question3]
if len(listdiff)==0: 
    print('question 3 done')
else:
    print('The difference question 3=', listdiff)


listdiff = [item for item in studentID if item not in question4]
if len(listdiff)==0: 
    print('question 2 done')
else:
    print('The difference question 2=', listdiff)
...
listdiff = [item for item in totalHrs if item not in question5]
if len(listdiff)==0: 
    print('question 5 done')
else:
    print('The difference question 5=', listdiff)


listdiff = [item for item in nameList if item not in question6]
if len(listdiff)==0: 
    print('question 6 done')
else:
    print('The difference question 6=', listdiff) 


listdiff = [item for item in nameActive10 if item not in question7]
if len(listdiff)==0: 
    print('question 7 done')
else:
    print('The difference question 7=', listdiff)


listdiff = [item for item in nameInAug if item not in question8]
if len(listdiff)==0: 
    print('question 8 done')
else:
    print('The difference question 8=', listdiff)
